...
let
  crossArchQemuPackage =
    ...
    // {
      stdenv = qemu.stdenv // {
        hostPlatform = qemu.stdenv.hostPlatform // {
          isLinux = true;
        };
      };
    };
  ...
  mkVmPackages =
    ...
    {
      homelab-vm =
        (config.flake.nixosConfigurations.homelab.extendModules { modules = vmModules; })
        .config.system.build.vm;
      homelab-vps-vm =
        (config.flake.nixosConfigurations.homelab-vps.extendModules { modules = vmModules; })
        .config.system.build.vm;
      desktop-vm =
        (config.flake.nixosConfigurations.desktop.extendModules { modules = vmModules; })
        .config.system.build.vm;
    };
in
{
  config.flake.packages = inputs.nixpkgs.lib.genAttrs [
    "x86_64-linux"
    "aarch64-darwin"
  ] mkVmPackages;
