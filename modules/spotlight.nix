    ...
    }:
    {
      home.activation = {
        rsync-home-manager-applications = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
          rsyncArgs="--archive --checksum --chmod=D+w,F-w --copy-unsafe-links --delete"
...
in
{
  ...
  config.darwin.modules.spotlight = { pkgs, config, ... }: {
    ...
    system.activationScripts.postActivation.text = ''
      ...
      app_target_base="${config.mine.homeDir}/Applications"
      app_target="$app_target_base/$moniker"
      mkdir -p "$app_target"
      ${pkgs.rsync}/bin/rsync --archive --checksum --chmod=D+w,F-w --copy-unsafe-links --delete "$apps_source/" "$app_target"
    '';
